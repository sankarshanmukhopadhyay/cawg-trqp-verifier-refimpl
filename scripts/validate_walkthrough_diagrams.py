#!/usr/bin/env python3
"""Validate Mermaid coverage and basic fence integrity for walkthrough docs."""
...
if len(linked) != 14:
    errors.append(f"walkthrough index must reference 14 unique documents, found {len(linked)}")
...
for path in linked:
    ...
    fences = text.count("```")
    if openings < 1:
        errors.append(f"{rel}: missing Mermaid diagram")
    if fences % 2:
        errors.append(f"{rel}: unbalanced fenced code blocks")
    if "## At-a-glance governance flow" not in text:
        errors.append(f"{rel}: missing standard at-a-glance flow heading")
...
print(f"walkthrough diagrams: {len(linked)}/14 OK")
