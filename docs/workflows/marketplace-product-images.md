...
```mermaid
sequenceDiagram
    participant S as Seller or Brand Delegate
    participant W as Marketplace Moderation Workflow
...
Introducing an agent into **Marketplace Product Images** changes the assurance problem from actor recognition alone to delegated-action assurance. Apply the common [Agentic AI Assurance model](../agentic-ai/index.md) and treat agent identity as necessary but insufficient evidence.

| Agentic control | Walkthrough requirement |
|---|---|
| Agent role | Model the agent explicitly as **producer, seller proxy, submitter, verifier, or marketplace moderation orchestrator**; authority in one role MUST NOT imply authority in another. |
| Principal | Bind the agent to **seller, brand, marketplace, or authorized agency** and retain the principal reference in the decision evidence. |
...
| Scope | Enforce **SKU/listing, source asset, allowed edits, channel, jurisdiction, campaign, and time window** as applicable to the transaction. |
...
An agent permitted to optimize product imagery is not thereby authorized to fabricate product attributes, approve compliance claims, or publish outside the seller/marketplace mandate.
